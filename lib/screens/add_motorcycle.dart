...
class _AddMotorcycleState extends State<AddMotorcycle> {
  ...
  ModelMotorcycle _modelMotorcycle = ModelMotorcycle();
      ...
      body: Column(
        children: [
           CustomManageButton(
            ...
            title: "Agregar Marca de la Moto",
            onTap: addBrand
          ),
          CustomManageButton(
            icon: Icons.my_library_add_rounded,
            title: "Agregar Modelo de la Moto",
            onTap: addModel
          ),
          CustomManageButton(
            icon: Icons.numbers,
            title: "Agregar Año de la Moto",
            onTap: addYear
          ),
          /* CustomManageButton(
  ...
  void addBrand() {
    final alert =  AddBrandMotorcycle();
    showDialog(context: context, builder: (_) => alert);
  }

  void addModel() async {

    final brands = await _brandMotorcycle.getBrandsMotorcycle();
    String brandId = '';
        ...
        TextButton(
          ...
          onPressed: () async {
            if (modelsController.text.isNotEmpty && brandId != '') {
              ...
              bool isDuplicate = await _modelMotorcycle.validateNoDuplicateRows(newname, newname.toLowerCase(), int.parse(brandId));
              if(isDuplicate){
                showDialog(
                context: context,
                builder: (c) {
                ...
                final lastId = await _modelMotorcycle.getLastIdRow();
                
                _modelMotorcycle.createModelMotorcycle(int.parse(brandId), lastId, newname, newname.toLowerCase());
